...
AI/RAG is **PARTIAL** (Issue 20 complete). The retrieval module now has: DocumentChunk model with tenant-scoped indexes, provider-neutral VectorStoreAdapter/KeywordAdapter ports with fake in-memory adapters, filter compiler (tenant/role/permission/department/category/classification filtering derived from auth context), RRF fusion engine, HybridRetrievalService, SUPER_ADMIN debug endpoint (`GET /retrieval/debug`), authenticated search endpoint (`POST /retrieval/search`), agent tool (`hybrid_search`), and audit events. LLM providers, chat module, citations, reranking, prompt construction, injection defense, refusal, confidence, bilingual validation, streaming, cancellation, fallback, token/cost/latency tracking, source viewer, and conversation persistence remain NOT IMPLEMENTED.
