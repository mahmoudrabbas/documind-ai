...
describe("AuthProvider logout", () => {
  it("always clears local session state after credentialed logout", async () => {
    ...
    expect(source).toContain("beginExplicitLogout()");
    expect(source).toContain("finishExplicitLogout()");
    expect(source).toMatch(/status: "unauthenticated",\s+user: null,\s+tenant: null,\s+accessToken: null/);
