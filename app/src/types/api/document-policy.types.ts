...
export interface ActivePolicyResponse { success: true; data: { policy: ActivePolicy; taxonomy: { categoryId: string | null; departmentId: string | null; classificationId: string | null }; mayManage: boolean } }
...
export interface PolicyPreview { documentId: string; currentPolicyId: string; currentPolicyVersion: number; proposedPolicyVersion: number; normalizedSummary: NormalizedPolicySummary; impact: PolicyImpact; sensitiveConfirmationRequired: boolean; previewToken: string; previewExpiresAt: string; previewFingerprint: string }
export interface PolicyPreviewResponse { success: true; data: PolicyPreview }
export interface PolicyApplyResult { status: "applied" | "no_change" | "idempotent_replay"; policyId: string; policyVersion: number; propagationEventId?: string | null }
...
export type PolicyErrorKind = "denied" | "unavailable" | "version_conflict" | "preview_expired" | "preview_invalid" | "preview_mismatch" | "sensitive_confirmation" | "idempotency_conflict" | "invalid_draft" | "invalid_reference" | "invalid_inheritance" | "batch_limit" | "taxonomy_duplicate" | "taxonomy_archived" | "network" | "unknown";
