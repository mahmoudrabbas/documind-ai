      ...
      {canCreate ? <DashboardPanel className="mb-6">
        ...
        <form id="invite" onSubmit={handleInvite} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="md:col-span-2 xl:col-span-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="submit" data-guide-id="users-invite-form-submit" disabled={submitting || loadingDepartments || Boolean(departmentError)} className="inline-flex min-w-40 items-center justify-center rounded-lg bg-primary px-5 py-2.5 font-bold text-on-primary disabled:opacity-50">{submitting ? t("dashboard.users.sendingInvite") : t("dashboard.users.sendInvitation")}</button>
          </div>
        </form>
      </DashboardPanel> : null}
