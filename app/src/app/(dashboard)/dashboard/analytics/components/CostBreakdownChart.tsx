...
export function CostBreakdownChart({ data }: CostBreakdownChartProps) {
  if (!data || data.length === 0) {
    return (
      <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-outline-variant/40 bg-surface-container-lowest p-6 text-center">
        <p className="text-body-md text-on-surface-variant">No model cost data available for the selected range.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-5 shadow-sm">
      <h3 className="text-title-md font-bold text-on-surface mb-4">Cost Breakdown by Model & Provider</h3>
        ...
        {data.map((item, idx) => (
          <div key={idx} className="flex items-center justify-between text-body-sm">
            ...
            <div className="flex items-center gap-4">
              <span className="text-on-surface-variant font-mono text-label-xs">{item.totalTokens.toLocaleString()} tokens</span>
              <span className="font-bold text-on-surface font-mono">${item.costUsd.toFixed(4)}</span>
              <span className="text-outline font-mono text-label-xs w-12 text-right">{item.percentageOfTotal}%</span>
            </div>
          </div>
        ))}
      </div>
