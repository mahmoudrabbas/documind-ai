...
export function SubscriptionOperationDialog({
  ...
  useEffect(() => {
    ...
    void previewSubscriptionImpact(tenantId, {
      ...
      if (!controller.signal.aborted) setError(caught instanceof ApiError ? caught.message : t("superAdmin.subsOperation.previewError"));
    }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [action, existing?.version, packageId, retry, t, targetStatus, tenantId]);
  ...
  async function confirm() {
    ...
    try {
      ...
      setError(caught instanceof ApiError
        ...
        : t("superAdmin.subsOperation.operationFailed"));
      ...
      <section ref={dialog} role="dialog" aria-modal="true" aria-labelledby="subscription-dialog-title"
        ...
        <div className="flex items-start justify-between gap-4">
          <h2 id="subscription-dialog-title" className="text-title-lg font-bold">
            {action === "provision" ? t("superAdmin.subsProvision") : t("superAdmin.subsOperation.confirmTitle")}
          </h2>
          <button ref={closeButton} type="button" aria-label={t("superAdmin.subsOperation.closeDialog")} disabled={pending} onClick={onClose}>✕</button>
        </div>
        {loading ? <p className="mt-4" aria-live="polite">{t("superAdmin.subsOperation.loadingPreview")}</p> : null}
        {!loading && error && !preview ? <div className="mt-4"><p role="alert">{error}</p><button type="button" className="mt-2 font-bold text-primary" onClick={() => setRetry((value) => value + 1)}>{t("superAdmin.subsOperation.retryPreview")}</button></div> : null}
        {preview ? <div className="mt-4 space-y-3 text-sm">
          <p><strong>{t("superAdmin.subsOperation.modeLabel")}</strong> {codeLabel(t, "superAdmin.operationMode", preview.operationMode)}</p>
          <p><strong>{t("superAdmin.companies.currentStatusLabel")}</strong> {preview.subscription?.status ? codeLabel(t, "superAdmin.subsStatus", preview.subscription.status) : t("superAdmin.subsOperation.noSubscription")}</p>
          <p><strong>{t("superAdmin.subsOperation.targetPackageLabel")}</strong> {preview.targetPackage ? t("superAdmin.subsOperation.targetPackageValue", { name: preview.targetPackage.name, version: String(preview.targetPackage.version) }) : t("superAdmin.subsOperation.unchanged")}</p>
          ...
          <label className="block font-bold">{t("superAdmin.subsOperation.reasonLabel")}
          ...
          <div className="flex justify-end gap-3">
            <button type="button" disabled={pending} onClick={onClose} className="rounded-lg px-4 py-2 font-bold">{t("common.cancel")}</button>
            <button type="button" disabled={pending || !preview.transitionAllowed} onClick={() => void confirm()}
              ...
              {pending ? t("superAdmin.subsOperation.applying") : t("common.confirm")}
            </button>
          </div>
        </div> : null}
      </section>
