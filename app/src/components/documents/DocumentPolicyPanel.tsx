...
import { DOCUMENT_ACCESS_ACTIONS, type ActivePolicy, type EffectiveAccessUser, type PolicyAssignment, type PolicyHistoryItem, type PropagationStatus } from "@/types/api/document-policy.types";
...
export function DocumentPolicyPanel({ document: doc }: { document: DocumentView }) {
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    setLoading(true); setError(null); setActive(null); setAssignments([]); setHistory([]); setPropagation(null); setDecisions([]);
    try {
      const current = await policyApi.getActivePolicy(doc.id, signal); if (signal?.aborted) return;
      setActive(current.data.policy);
      const [assignmentResult, historyResult, propagationResult] = await Promise.all([
        policyApi.getPolicyAssignments(doc.id, signal), policyApi.getPolicyHistory(doc.id, undefined, signal), policyApi.getPropagationStatus(doc.id, signal),
      ]);
      if (signal?.aborted) return;
      setAssignments(assignmentResult.data.assignments); setHistory(historyResult.data.policies); setHistoryCursor(historyResult.data.nextCursor); setPropagation(propagationResult.data);
  ...
  async function loadMoreHistory() { if (!historyCursor) return; try { const result = await policyApi.getPolicyHistory(doc.id, historyCursor); setHistory((current) => [...current, ...result.data.policies]); setHistoryCursor(result.data.nextCursor); } catch (cause) { setError(policyApi.classifyPolicyError(cause)); } }
  ...
  return <section className="mt-6 border-t border-outline-variant/30 pt-5" aria-labelledby="access-policy-title">
    ...
    {!loading && !error && active && <div className="mt-4">
      {tab === "overview" && <div className="grid gap-3 sm:grid-cols-2"><Info label="Sensitivity"><ClassificationBadge level={doc.classification} /></Info><Info label="Category">{doc.category ?? "Not assigned"}</Info><Info label="Department">{doc.department ?? "Not assigned"}</Info><Info label="Owner">{doc.owner ?? "Unavailable"}</Info><Info label="Active policy version">{active.policyVersion}</Info><Info label="Propagation">{propagation?.status ?? "Unavailable"}</Info></div>}
