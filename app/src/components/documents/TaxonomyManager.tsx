...
export function TaxonomyManager() {
  ...
  function closeDialog() { setForm(null); setConfirm(null); setFormError(null); window.setTimeout(() => triggerRef.current?.focus(), 0); }
  async function submit() {
    ...
    } catch (cause) { const kind = policyApi.classifyPolicyError(cause); setFormError(kind === "taxonomy_duplicate" || kind === "taxonomy_archived" ? "taxonomy.duplicateError" : kind); }
    ...
    <section aria-labelledby="taxonomy-heading" className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest shadow-sm">
      <div className="border-b border-outline-variant/30 p-4 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {canMutate && <Button data-guide-id="taxonomy-create-button" onClick={(event) => { triggerRef.current = event.currentTarget; setForm({ record: null, name: "", description: "", level: "internal" }); }}><span className="material-symbols-outlined me-1 text-[16px]" aria-hidden="true">add_circle_outline</span>{t("taxonomy.create")}</Button>}
        </div>
      ...
      {!loading && !error && records.length > 0 && <div className="overflow-x-auto" data-guide-id="taxonomy-table"><table className="w-full min-w-[680px] text-body-sm"><thead><tr className="border-b border-outline-variant/30 bg-surface-container-low"><th className="p-4 text-start text-label-md font-semibold text-on-surface-variant">{t("taxonomy.name")}</th><th className="p-4 text-start text-label-md font-semibold text-on-surface-variant">{t("taxonomy.status")}</th><th className="p-4 text-start text-label-md font-semibold text-on-surface-variant">{t("taxonomy.version")}</th><th className="p-4 text-start text-label-md font-semibold text-on-surface-variant">{t("taxonomy.updated")}</th><th className="p-4 text-end text-label-md font-semibold text-on-surface-variant">{t("documents.tableActions")}</th></tr></thead><tbody>{records.map((record) => <tr key={record.id} className="border-b border-outline-variant/20 transition-colors hover:bg-surface-container-low/50"><td className="p-4 align-top"><span className="font-semibold text-on-surface">{record.name}</span>{record.level && <div className="mt-1"><ClassificationBadge level={record.level} /></div>}</td><td className="p-4 align-top"><Badge status={record.status === "active" ? "success" : "neutral"} label={t(`taxonomy.${record.status}`)} /></td><td className="p-4 align-top text-on-surface-variant">{record.version}</td><td className="p-4 align-top text-on-surface-variant">{new Date(record.updatedAt).toLocaleDateString(intlLocale)}</td><td className="p-4 align-top text-end">{canMutate ? <div className="flex items-center justify-end gap-2"><Button size="sm" variant="ghost" className="h-9 gap-1.5 px-3 text-label-sm font-medium text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface" onClick={(event) => { triggerRef.current = event.currentTarget; setForm({ record, name: record.name, description: record.description ?? "", level: record.level ?? "internal" }); }}><span className="material-symbols-outlined text-[16px]" aria-hidden="true">edit</span>{t("taxonomy.edit")}</Button><Button size="sm" variant="outline" className={`${record.status === "active" ? "border-error/30 bg-surface-container-lowest text-error hover:border-error/40 hover:bg-error/5" : "border-outline-variant/40 bg-surface-container-lowest text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface"} h-9 gap-1.5 px-3 text-label-sm font-medium`} onClick={(event) => { triggerRef.current = event.currentTarget; setConfirm({ record, action: record.status === "active" ? "archive" : "restore" }); }}><span className="material-symbols-outlined text-[16px]" aria-hidden="true" style={record.status === "active" ? { fontVariationSettings: "'FILL' 1" } : undefined}>{record.status === "active" ? "archive" : "unarchive"}</span>{t(`taxonomy.${record.status === "active" ? "archive" : "restore"}`)}</Button></div> : <span className="text-body-sm text-on-surface-variant">{t("taxonomy.readOnly")}</span>}</td></tr>)}</tbody></table></div>}
