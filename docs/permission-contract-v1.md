...
Phase 5 additively defines `documents:manage-access` and `documents:use-in-ai` for Super/Company Admin defaults. Both are tenant-delegable with document scopes. They remain independent: read does not imply AI use, and update does not imply access-policy management. The document policy is always the narrowing authority.

`documents:use-in-ai` is deliberately not an `EMPLOYEE` default. A base-role default is an unconditional inherited grant that a custom role can narrow but never revoke, so defaulting it for `EMPLOYEE` made the role editor's "Use Documents in AI" checkbox unable to deny anything, and left a document access policy granting `use_in_ai` as the only remaining gate — a policy could then hand AI access to a role that was never given the capability. Employees must be granted it explicitly; it stays fully grantable to `EMPLOYEE` custom roles because `allowedCustomRoleBases` derives from `tenantGrantable`, not from `defaultBaseRoles`.
