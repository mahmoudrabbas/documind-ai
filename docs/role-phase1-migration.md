...
Pending or otherwise incomplete users cannot create or rotate refresh sessions. Session issuance re-reads and write-locks the authoritative user inside the same transaction that inserts the refresh record; blocked requests return `AUTH_SESSION_MIGRATION_PENDING` without exposing migration details. An active refresh session is any tenant-and-user-scoped record whose `revokedAt` is null or missing. Expiry is intentionally ignored for this invariant, so missing or malformed legacy expiry data fails closed.

Final completion takes the same per-user write lock, revokes every active session, queries again for active sessions, and changes the expected pending state to `complete` in one transaction. If the zero-session invariant cannot be proven, the transaction aborts with `ACTIVE_SESSIONS_REMAIN`; the user remains pending and a rerun safely retries the complete operation. Operators should stop unexpected session issuers, revoke the identified user's sessions through the approved session controls, and rerun from the last successful checkpoint. No migrated user is validly complete while an active legacy refresh session exists.
