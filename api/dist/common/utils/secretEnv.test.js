...
test("a configured missing secret file fails without exposing fallback content", () => {
    ...
    try {
        assert.throws(() => getSecretValue("JWT_SECRET"), (error) => error instanceof Error && error.message === "Unable to load configured secret: JWT_SECRET_FILE" && !error.message.includes("fallback-must-not-be-used"));
    }
    finally {
        delete process.env.JWT_SECRET_FILE;
        delete process.env.JWT_SECRET;
...
//# sourceMappingURL=secretEnv.test.js.map