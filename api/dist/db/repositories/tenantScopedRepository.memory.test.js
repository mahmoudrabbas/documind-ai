...
import { tenantScopedCreate, tenantScopedDeleteOne, tenantScopedFindById, tenantScopedFindOne, tenantScopedUpdateOne, } from "./tenantScopedRepository.js";
const tenantScopedSchema = new Schema({
    tenantId: { type: String, required: true },
    email: String,
    userId: String,
    status: String,
}, { timestamps: false });
const TenantScopedModel = mongoose.models.TenantScopedTest ||
    mongoose.model("TenantScopedTest", tenantScopedSchema);
let mongoServer;
before(async () => {
...
test("tenant scoped repository enforces tenant isolation with MongoMemoryServer", async (t) => {
    ...
    await t.test("tenantScopedFindOne only returns results for the requested tenant", async () => {
        ...
        const result = await tenantScopedFindOne(TenantScopedModel, "tenant-a", { email: "shared@example.com" }).exec();
    ...
    await t.test("tenantScopedFindById honors tenant boundary", async () => {
        ...
        const found = await tenantScopedFindById(TenantScopedModel, "tenant-a", tenantARecord.id).exec();
        const notFound = await tenantScopedFindById(TenantScopedModel, "tenant-b", tenantARecord.id).exec();
        assert.ok(found);
    ...
    await t.test("tenantScopedUpdateOne only updates the matching tenant document", async () => {
        ...
        const updateResult = await tenantScopedUpdateOne(TenantScopedModel, "tenant-a", { userId: "shared-user" }, { $set: { status: "inactive" } }).exec();
    ...
    await t.test("tenantScopedDeleteOne only deletes the matching tenant document", async () => {
        ...
        const deleteResult = await tenantScopedDeleteOne(TenantScopedModel, "tenant-a", { userId: "shared-user" }).exec();
...
//# sourceMappingURL=tenantScopedRepository.memory.test.js.map