...
export async function updateSubscription(tenantId, input, actor) {
    ...
    if (!pkg)
        throw new AppError(404, "NOT_FOUND", "Active package not found");
    const value = await SubscriptionModel.findOneAndUpdate({ tenantId }, {
        $set: {
            packageId: pkg._id,
...
export async function getSetting(key) {
    return ((await PlatformSettingModel.findOne({ key }).lean().exec())?.value ?? {});
}
