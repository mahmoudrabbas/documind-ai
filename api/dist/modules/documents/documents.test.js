...
async function login(port, slug = "acme-consulting", email = "sarah@acme.com") {
...
function buildMultipartBody(fileName, fileContent, metadata) {
...
void test("POST /documents — upload a document successfully", async () => {
    ...
    assert.equal(doc.metadata.title, "Annual Report 2024");
...
void test("GET /documents — returns empty list when no documents", async () => {
    ...
    assert.equal(data.pagination.totalRecords, 0);
    await closeServer(server);
});
void test("GET /documents — paginated list", async () => {
    ...
    assert.equal(data.pagination.totalRecords, 3);
...
void test("GET /documents/:id — returns single document", async () => {
    ...
    assert.equal(response.status, 200);
    const body = (await response.json());
    const result = body.data.document;
...
void test("PATCH /documents/:id — updates metadata", async () => {
    ...
    const body = (await response.json());
    const result = body.data.document;
    assert.equal(result.metadata.title, "Updated Title");
    assert.deepEqual(result.metadata.tags, ["new", "tags"]);
    await closeServer(server);
});
