...
async function installResetToken(tenantId, userId) {
    const token = createPasswordResetToken({ tenantId, userId });
    await UserModel.updateOne({ _id: userId, tenantId }, { $set: { passwordResetTokenHash: token.tokenHash, passwordResetExpiresAt: token.expiresAt } });
    return token;
}
...
test("a token cannot be used with another tenant slug and is not consumed", async () => {
    ...
    const tokenA = await installResetToken(tenantA._id.toString(), userA._id.toString());
    await assert.rejects(resetPassword({ token: tokenA.token, slug: "beta-company", password: "newPassword1", confirmPassword: "newPassword1" }), /Invalid or expired password reset token/);
    const [storedA, storedB] = await Promise.all([
        UserModel.findById(userA.id).select("+passwordHash +passwordResetTokenHash"),
...
//# sourceMappingURL=password-reset-tenant-isolation.test.js.map