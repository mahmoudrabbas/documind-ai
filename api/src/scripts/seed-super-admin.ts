...
async function main() {
  if (process.env.SEED_SUPER_ADMIN_ENABLED?.toLowerCase() !== "true") {
    throw new Error(
      "Super Admin seed is disabled. Set SEED_SUPER_ADMIN_ENABLED=true explicitly.",
    );
  }
  await connectDB();
  ...
  console.info(
    `Super Admin seed completed for ${result.email}; no password was logged.`,
  );
}
...
main()
  .catch((error: unknown) => {
    console.error(
      error instanceof Error ? error.message : "Super Admin seed failed",
    );
    process.exitCode = 1;
  })
