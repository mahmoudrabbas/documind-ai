...
test("catalog identifiers are normalized, unique, versioned, and cover all twelve groups", () => {
  ...
  assert.deepEqual([...new Set(PERMISSION_CATALOG.map((item) => item.group))].sort(), ["analytics", "audit", "billing", "chat", "company-settings", "documents", "feedback", "imports", "knowledge-gaps", "notifications", "roles", "users"]);
