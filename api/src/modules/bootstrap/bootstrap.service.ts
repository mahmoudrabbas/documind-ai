...
export async function bootstrapSuperAdmin(body: unknown, key: string | undefined) {
  ...
  if (await UserModel.exists({ role: "SUPER_ADMIN" })) throw new AppError(409, "SUPER_ADMIN_ALREADY_CONFIGURED", "The initial Super Admin has already been configured.");
  if (await UserModel.exists({ email: input.email })) throw new AppError(409, "EMAIL_ALREADY_IN_USE", "The email cannot be used for platform administration.");
