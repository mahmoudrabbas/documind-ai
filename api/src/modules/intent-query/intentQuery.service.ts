...
export class IntentQueryService {
  ...
  async analyzeQuery(
    ...
    // authorized documents forces a clarification — never guess. An unresolved
    // hint (a referenced title that matches no authorized retrievable
    // document) is equally a clarification signal: searching with it would
    // reveal whether an inaccessible document exists, so retrieval is not
    // attempted.
