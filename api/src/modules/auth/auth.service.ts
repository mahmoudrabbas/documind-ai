  ...
  try {
    try {
      ...
      if (isTransactionUnsupported) {
	    ...
	    // Provision subscription after tenant+user creation — non-blocking
	    try {
	      await provisionSubscription(created.tenant._id.toString(), payload.packageCode);
	    } catch (error) {
	      console.error("[auth-register] Failed to provision subscription:", error);
	    }
...
export async function completeTrial(
  ...
  // Transition the existing TRIALING subscription to ACTIVE
  const subscription = await transitionSubscription(
    identity.tenantId,
    "ACTIVE",
    { reason: "trial_completed", actor: identity },
  );

  // Resolve package name for the response
  const pkg = await getPackage(subscription.packageId.toString());
